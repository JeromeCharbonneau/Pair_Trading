...
Last updated: 2026-07-30 (local) — Bloomberg is primary research source; Yahoo is provisional comparison only
...
## Bloomberg status (ingested — primary)

Bloomberg workbook is **already ingested** at `data/raw/2026-07-29T234058Z_bloomberg/`.

- Clean primary panel: `data/clean/corn_wheat_panel_bloomberg.*` + `lineage_bloomberg.json`
- Day 3–4 features/models/backtests use Bloomberg as the **primary** research series.
- Yahoo remains a **provisional comparison** / Day-1 screen only.
- **Still disclosed:** Bloomberg sheets are vendor continuous series; roll dates/method are not reconstructed from per-contract Panama rules. True per-contract rolls remain a future improvement if contract-level history becomes available.

Environment pin: see `requirements.txt`.

## Day 2–4 status

- Clean Corn/Wheat panels + lineage: `data/clean/` (`lineage.json` Yahoo provisional; `lineage_bloomberg.json` primary)
- Data-quality memo: `docs/DATA_QUALITY_MEMO.md`
- Features: `data/features/` — see `docs/FEATURES.md`
- Day 4 backtest: `data/results/` — see `docs/DAY4_METHOD.md`, `docs/DAY4_RESULTS.md`, `docs/RESEARCH_LOG.md`
- Full Panama roll from expired contracts: still not available from Yahoo; Bloomberg continuous remains vendor-defined

## Pair selection note

Working pair: **Corn / Wheat**, chosen for economic link + cointegration + return correlation. Research series for Days 3–4 is **Bloomberg**; Yahoo `=F` was the provisional screen.
